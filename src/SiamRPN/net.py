...
class SiamRPN(nn.Module):
    ...
    def update_temple(self, z, alpha):
        z_f = self.featureExtract(z)
        r1_kernel_raw = self.conv_r1(z_f)
        cls1_kernel_raw = self.conv_cls1(z_f)
        kernel_size = r1_kernel_raw.data.size()[-1]
        new_r1_kernel = r1_kernel_raw.view(self.anchor*4, self.feature_out, kernel_size, kernel_size)
        self.r1_kernel = (1 - alpha) * self.r1_kernel + alpha * new_r1_kernel
        new_cls1_kernel = cls1_kernel_raw.view(self.anchor*2, self.feature_out, kernel_size, kernel_size)
        self.cls1_kernel = (1 - alpha) * self.cls1_kernel + alpha * new_cls1_kernel

# configs = []
# for lr in [0.295, 0.3, 0.4]:
#     for window_influence in [0.40, 0.42, 0.44, 0.46]:
#         for penalty_k in [0.055, 0.04, 0.1, 0.2]:
#                     configs.append({'lr': lr, 'window_influence': window_influence, 'penalty_k': penalty_k, 'instance_size': 271, 'adaptive': True})
